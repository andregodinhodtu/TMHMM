...
print(f"SANITY CHECK: Downsize flag is set to -> {downsize}")
print(f"SANITY CHECK: Train proportion is set to -> {train_prop}")
...
if downsize:
    train_output_file = f"train_dataset_fused_&_downsized_{train_prop*100}%.txt"
    test_output_file = f"test_dataset_fused_&_downsized{test_prop*100}%.txt"
else:
